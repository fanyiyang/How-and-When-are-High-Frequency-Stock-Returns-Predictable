...
def volume_clock(volumecum):
    """Map cumulative volume onto a timedelta axis (1 share = 1 second).

    pandas only does value-based rolling windows on datetime-like indexes; an
    integer window on a numeric index counts rows (i.e. trades). Expressing
    cumulative volume as timedeltas lets ``rolling("1000s")`` mean "the last
    1000 shares traded" rather than "the last 1000 trades".
    """
    return pd.to_timedelta(volumecum, unit="s")


def _is_zero_window(window):
    if isinstance(window, str):
        return pd.Timedelta(window) == pd.Timedelta(0)
    return window == 0


def rolling_window_diff(series, window_left, window_right, agg):
    """Aggregate ``series`` over the trailing window [t - window_right, t - window_left).

    pandas has no native "between two lags" window, so take the difference of two
    left-closed rolling aggregates. A zero-width ``window_left`` must be skipped
    entirely: rolling over an empty window returns NaN, which would poison the
    difference for every row.
    """
    right = getattr(series.rolling(window_right, closed="left"), agg)()
    if _is_zero_window(window_left):
        return right
    left = getattr(series.rolling(window_left, closed="left"), agg)()
    return right - left
...
def build_feature_set(stockname):
    ...
    def response_calculator(data):
        ...
        # Volume-clock returns: mean price over the next 1000/20000 *shares*
        # traded, not the next 1000/20000 trades.
        price_on_volume = pd.Series(data.Price.values, index=volume_clock(data.index))
        data.Return1000Volumes = (
            price_on_volume.rolling("1000s").mean().values / data.Price.values - 1
        )
        data.Return20000Volumes = (
            price_on_volume.rolling("20000s").mean().values / data.Price.values - 1
        )
    ...
    def predictor_calculator(data, timetype, timescale):
        ...
        if timetype == "calendar":
            window_left = f"{int(round(time_cal[timescale][0] * 1000))}ms"
            window_right = f"{int(round(time_cal[timescale][1] * 1000))}ms"
            time_window = data.Time
            scale_length = time_cal[timescale][1] - time_cal[timescale][0]
        if timetype == "transaction":
            ...
            scale_length = time_tran[timescale][1] - time_tran[timescale][0]
        if timetype == "volume":
            window_left = f"{time_vol[timescale][0]}s"
            window_right = f"{time_vol[timescale][1]}s"
            time_window = volume_clock(data.Volumecum).rename("VolumeClock")
            scale_length = time_vol[timescale][1] - time_vol[timescale][0]
        data.index = time_window
        breadth = rolling_window_diff(data.Price, window_left, window_right, "count")
        immediacy = scale_length / breadth
        volume_all = rolling_window_diff(data.TradeQty, window_left, window_right, "sum")
        volume_avg = volume_all / breadth
        plt = data.Price - data.Price.shift(1)
        lambd = rolling_window_diff(plt, window_left, window_right, "sum") / volume_all
        ispread = (data.OfferSize1 - data.BidSize1) / (data.OfferSize1 + data.BidSize1)
        lob_imbalance = rolling_window_diff(ispread, window_left, window_right, "sum") / breadth
        direction = pd.Series(np.where(data.Direction == Side.BUY, 1.0, -1.0), index=data.index)
        txn_imbalance = rolling_window_diff(
            data.TradeQty.multiply(direction), window_left, window_right, "sum"
        ) / volume_all
        past_return = 1 - rolling_window_diff(
            data.Price, window_left, window_right, "sum"
        ) / breadth / data.Price
        ...
        auto_cov = rolling_window_diff(
            log_plt1.multiply(log_plt2), window_left, window_right, "sum"
        ) / breadth
        spread = (data.OfferPX1 - data.BidPX1) / (data.OfferPX1 + data.BidPX1)
        quoted_spread = rolling_window_diff(spread, window_left, window_right, "sum") / breadth
        w_spread = log_plt1.multiply(direction).multiply(data.TradeQty).multiply(data.Price)
        weight = data.TradeQty.multiply(data.Price)
        effective_spread = rolling_window_diff(
            w_spread, window_left, window_right, "sum"
        ) / rolling_window_diff(weight, window_left, window_right, "sum")
        predictor = pd.concat(
            {
    ...
    def build_dataset(data_original, mode="calendar"):
        ...
        if mode == "calendar":
            ...
            xset = pd.concat(list(predictor_dict.values()), axis=1)
            ...
            xset = pd.concat(list(predictor_dict.values()), axis=1)
            # data's index is already the volume clock set by predictor_calculator,
            # so the labels align with xset without re-indexing.
            yset = data[["Return1000Volumes", "Return20000Volumes"]]
