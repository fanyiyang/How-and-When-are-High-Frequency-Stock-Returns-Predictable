...
#读取数据
# stockname='000333'#'000858','002024','300144','000786'      '300413','300750','300015','002466','300003','300628','000568','000651','000157','000338','002607','000776','002714','000876','000786',
for stockname in ['000333']:
    print(stockname)
    ...
    def train_model(model, X_train, y_train, X_test, y_test):
        ...
        if model.__class__.__name__ == 'LGBMRegressor' and False:
            ...
            plt.legend()
            plt.show()
        if model.__class__.__name__ == 'MLPRegressor':         # 绘制损失函数下降图
    ...
    def RFscore(trial):
        ...
        for i in range(3):
            ...
            clf_rf=RandomForestRegressor(n_estimators=50,max_samples=0.1,max_depth=x,random_state=42) ###
    ...
    def NNscore(trial):
        ...
        for i in range(3):
            X_train, y_train1, y_train2, X_test, y_test1, y_test2 = process_data(Xset, yset1, yset2, today_index, i)
            y_train1, y_train2,y_test1, y_test2 = 10000*y_train1, 10000*y_train2, 10000*y_test1, 10000*y_test2
    ...
    for i in range(20,len(all_date)-20,20):
        ...
        for j in tqdm(range(20)):
            try:
                ...
                # 获取被选中的变量# selected_features = [feature for feature, coef in zip(feature_names, lasso.coef_) if coef != 0]
                feature_importance['lasso'].iloc[i+j-20] = lasso.coef_
                
                # RF
                clf_rf=RandomForestRegressor(n_estimators=50,max_samples=0.1,max_depth=RFpara)
                ...
                # xgb = XGBRegressor(subsample=0.01, max_depth=XGBpara, random_state=42)
                # xgb.fit(x_sample, y_sample1)
                # y_predxgb1 = xgb.predict(x_outsample)
                # Rset.loc[all_date[i+j], 'XGB5sR^2'] = r2_score(y_outsample1, y_predxgb1)
                # xgb.fit(x_sample, y_sample2)
                # y_predxgb2 = xgb.predict(x_outsample)
                # Rset.loc[all_date[i+j], 'XGB30sR^2'] = r2_score(y_outsample2, y_predxgb2)
                # end_time_xgb = time.time()
                # print(f"Time taken for XGBoost: {end_time_xgb - start_time_xgb}s")
                ...
                # nn = MLPRegressor(hidden_layer_sizes=(NNpara,NNpara),tol = 1e-5,max_iter=1000)
                # nn.fit(x_sample, y_sample1*10000)
                # y_prednn1 = nn.predict(x_outsample)/10000
                # # sns.distplot(y_prednn1)
                # # sns.distplot(y_outsample1)
                # Rset.loc[all_date[i+j], 'NN5sR^2'] = r2_score(y_outsample1, y_prednn1)
                # nn.fit(x_sample, y_sample2*10000)
                # y_prednn2 = nn.predict(x_outsample)/10000
                # Rset.loc[all_date[i+j], 'NN30sR^2'] = r2_score(y_outsample2, y_prednn2)
                # end_time_nn = time.time()
                # print(f"Time taken for 2 layer neural network: {end_time_nn - start_time_nn}s")
